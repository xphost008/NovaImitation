<script setup lang="ts">
    ...
    const isCheckedProps = withDefaults(defineProps<CheckButtonProps>(), {isExpand: false, maxHeight: 0, title: "", isLast: false})
...
<template>
    <div>
        <div :class="'grid' + (isCheckedProps.isExpand ? ' cursor-pointer' : '')" :isOpen="(isExpandComp ? 'expand' : 'close')" @click="changeProps">
            ...
            <svg
                ...
                fill="none" :class="isExpandComp ? 'card-icon-expand' : 'card-icon'" :style="isCheckedProps.isExpand ? 'display: flow' : 'display: none'">
